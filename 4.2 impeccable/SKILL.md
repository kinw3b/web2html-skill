...
## FIDELITY LOCK — when called from `website-to-html` Step 8

If the parent is **`website-to-html`** polish (or the target is a Paper-exported rebuild):

1. Prefer **`audit` + `polish` only**. Do **not** run `craft`, `shape`, `bolder`, `overdrive`, `teach` redesign loops, `typeset`, or `layout`.
2. PRODUCT.md missing → skip teach; audit/polish against existing `rebuild/` + `DESIGN.md` only.
3. **Do not** change section order, brand colors, fonts, font-size, `--text-*`, gap, padding, radius, or Design Library class names. 2.3 already signed those. 2.4 froze them (`qa/fidelity-freeze-24.json`).
4. Allowed: a11y contrast / `:focus-visible` on **existing** controls (recolor the **label** to an existing token if WCAG fails — never bump type size), residual a11y defects, console errors that do not require a restyle, anti-AI-slop (invented chrome, peeking skip-links). **Not allowed:** Paper visual restore (icons, checks, measured gaps, text-align). That is 2.3 only (Pitfall #196). Do **not** load `/pixel-perfect` in 3.x. Load `website-to-html/references/polish-visual-restore.md`.
5. If the page is a freehand redesign of the Paper capture, **report fidelity failure** and stop redesigning — the fix is 2.3, not more polish.
6. **Do not invent chrome.** No skip-link, skip-to-content, “back to top”, or any new control Paper did not paint. `translateY` off-screen still peeks (Pitfall #81 / prior-run). If a skip-link already exists in the source, clip it with `clip` + `clip-path: inset(50%)` until `:focus-visible` — never a visible-at-rest pill.
