...
On a Paper rebuild: GSAP in-view stagger is **mandatory** at 3.2 (`inject-gsap-reveal.py` on `index-polish.html`). Do not skip it because 1.4 did not record motion (Pitfall #204). A painted burger drawer is also mandatory (`author-nav-drawer.py`); do not list it as a skip for Capture Tool (Pitfall #208). Painted FAQ accordion (`author-faq.py`) and nav dropdowns (`author-nav-dropdown.py`) are also mandatory; do not skip empty Paper bodies or `--allow-dropdown` (Pitfall #209 #210). Other extras: only motion that 1.4 already recorded, or a focus/easing/reduced-motion gap. Cap the extras list. Do not implement here — Emil injects GSAP, hover CSS, the burger drawer, FAQ, and dropdowns. **Required artifact:** write the opportunities table plus every rejected candidate (with the lock that rejected it) to `qa/find-animation-opportunities.md`. A list that only lives in the reply is a dropped finding — `verify-polish-passes.py` and `mark --step 3.2 --status done` fail without that file (Pitfall #215).
