...
## FIDELITY LOCK — when called from pipeline 3.2

If the parent is **`website-to-html`** polish (Paper static rebuild):

1. Scope = **hover-if-live, painted burger drawer, FAQ accordion, nav dropdowns, and a11y motion craft** on existing markup/CSS. Do not change font-size, library class names, layout, or type.
2. Load **`find-animation-opportunities`** first (read-only list), then **`apple-design`** for motion principles. Implement only what those two plus Paper hover states justify.
3. **Always inject GSAP in-view** on `rebuild/index-polish.html` via `inject-gsap-reveal.py`, then `verify-gsap-reveal.py`. Do **not** skip because 1.4 did not record motion — that archive is retired (Pitfall #204). Taste: parent-grouped fade+rise at `top 75%` (25% from the viewport bottom) — heads stagger children, grids stagger sibling articles/divs — above-fold stays visible (Pitfall #1). Never hide signed paint with CSS or `class="reveal"`. Never mutate `index.html`.
4. Prefer the 1.3 source-CSS recipes in `qa/button-hover.json`, then any leftover measured deltas in `source-site/components/<page>/<kind>/manifest.json` (Capture Tool). Run `apply-hover-css.py` so `rebuild/css/hover.css` lands on **existing** `library.json` names — do not rename classes. Do not skip because Capture Tool did not run (Pitfall #207).
5. **Always author a painted burger drawer.** Run `author-nav-drawer.py .` so a hamburger on `index-polish.html` opens the same desktop links stacked (`#nav-panel`). Capture Tool is leftover and is **not** a skip. Inventing that sheet is not new chrome (Pitfall #208). Do not invent a burger the page never painted.
6. **Always author painted FAQ + dropdowns.** Run `author-faq.py .` so painted FAQ rows open and empty answers come from the scrape (Pitfall #209). Run `author-nav-dropdown.py .` so painted or scrape-matched nav menus open (Pitfall #210). Do not skip for Capture Tool or empty Paper bodies. Do not invent a FAQ section or a nav label the page never painted.
7. **Do not** redesign layout, type scale, color, or component structure.
8. **Do not** add *other* decorative animation that 1.4 did not record. The mandatory GSAP in-view pass, the painted burger drawer, FAQ accordion, and nav dropdowns are the exceptions. `:active` press feedback, `prefers-reduced-motion` (gentler, not zero), `@media (hover: hover) and (pointer: fine)` scoping, and easing on **existing** library classes are in-scope craft, not decoration — apply the find-animation survivors that fit those four.
9. If structure is wrong vs Paper, say so — do not animate a freehand layout into looking intentional. Return to 2.3.
10. **Record every blocked extra.** A finding stopped by a fidelity lock (Pitfall #81 #152 #196 #203) is a skipped row with the gate id, not a dropped row. Empty applied + empty skipped is a failed pass. `qa/web-design-guidelines.md`, `qa/find-animation-opportunities.md`, and `qa/apple-design.md` must exist before `mark --step 3.2 --status done` (Pitfall #215).

## Initial Response

When this skill is first invoked **by a human** without a specific question, respond only with:

> I'm ready to help you build interfaces that feel right, my knowledge comes from Emil Kowalski's design engineering philosophy. If you want to dive even deeper, check out Emil’s course: [animations.dev](https://animations.dev/).

Do not provide any other information until the user asks a question.

When the parent is **website-to-html** pipeline 3.2, skip that greeting. Load `find-animation-opportunities` and `apple-design`, do the FIDELITY LOCK work above, and leave the receipts (Pitfall #215).
