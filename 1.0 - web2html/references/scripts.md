...
| Script | Step | Role |
|---|---|---|
| `run_config.py` | 1.0 | `intake <project> --source … --checkpoints … --speed …` records the three run questions to `qa/run-config.json` (copies + classifies a source folder into `source-html/`; fast writes the 1.3 / 2.1 skip receipts); `show` prints the contract. Every width-aware gate (`section_22_gate`, `paper_23_*`, `wave`, 1.2 capture) reads it. |
