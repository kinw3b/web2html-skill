...
| `rebuild/` | Steps **2a–9** | **Ship** — only after Stage L passes. `index-semantic.html` is the 2.2 first pass; `index.html` is the 2.4 lock; `index-polish.html` is 3.x QA. Do not write `NEXT.html` |
...
| 9 | Browser | Chrome on `file://` by default; HTTP **user-opt-in** only (Pesticide); never Pencil |
...
| **2.3** | **Validate each section vs Paper** | **Never skip.** Seed `index.html` from `index-semantic.html`. Pull numbered 1.2 `NN-slug.png` clips at 1600 / 768 / 390 (`paper_23_clip_compare.py`) and compare them to rebuild shots + `index-raw.html`. Serial or max two workers. No Paper MCP. Then the VALIDATE walk (`paper_23_validate.py`: shoot → Read the side PNGs → patch → record, ≤3 rounds per band, top to bottom). `clip-compare.json` + `disk-gold.json` + `raw-census.json` + every `<id>.validate.json` (last round match or capped residual, ship unchanged since that round was shot) + `section_22_gate.py` green. Pitfall #185 #195 #198 #201 #202 #216. |
