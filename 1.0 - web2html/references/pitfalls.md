...
| 187 | 1.2 opened an existing Paper file because the name looked similar (Thrive / kp-thrive leftover) | `capture-session.mjs` always `create_file` a timestamped new document. Omit `--file`. Never `list_files` / never `open_file` a similarly-named existing file. Pin `qa/paper-file.json` from **this** create for the rest of the run only. |
...
| 195 | 2.3 loaded `/pixel-perfect` as the driver and ran its refine loop on every band (hour+) | 2.3 self-validates each section vs disk gold. Serial, or at most two workers. Pixel-perfect is a **one-pass assist** for layout / type / geometry / icons / radius / imagery / absolute overlays / 2.2 hallucinations. Cap 1 compare + 1 fix per section **for the assist**; the controller's own VALIDATE walk is separate and capped at 3 looked-at rounds per band (Pitfall #216). Workers do not write `rebuild/`. Recipe `references/section-23-paper-loop.md`. |
...
| 202 | 2.3 spawned one Paper-bound worker per section; desktop MCP timed out and the batch never returned | Gold is already on disk: 1.2 `capture/home-{desktop,768,390}/source-sections/NN-slug.png` (same number at every breakpoint) plus `rebuild/index-raw.html`. Run `paper_23_disk_gold.py` then `paper_23_clip_compare.py` so those numbered clips land in `qa/paper-measure/compare/` as source / rebuild / side-by-side. Read those PNGs. Do not compare against 1.1 `source-site/screenshots/` (that is 3.x). Default is serial. Spawn at most two disk-only workers. No Paper MCP. Recipe `references/section-23-paper-loop.md`. |
...
| 216 | 2.3 signed `aligned` after one compare; stacked columns, wrong band order, and dropped overlays surfaced at 2.4 and were fixed by hand from screenshots, one section at a time | After APPLY, run the **VALIDATE walk** top to bottom (`paper_23_validate.py`): `--next` names the open band, `--id <band> --shoot` re-shoots it at 1600 / 768 / 390 and rebuilds the side-by-sides, **Read all three `NN-<band>-{width}-side.png`**, compare with the 1.2 clip + `index-raw.html`, patch that band only, `--record --seen … --verdict … --miss … --patched`, repeat until every width is `match` or round 3, then `--residual`. Controller only, no Paper MCP, no pixel-perfect inside the walk. `section_22_gate.py` fails a band without `qa/paper-measure/<id>.validate.json`, with an empty `seen`, an open last round, more than 3 rounds, a residual before round 3, or a ship (`rebuild/index.html` + `rebuild/css`, minus the QA overlay and 3.x sheets) that changed after the last round was shot. Recipe `references/section-23-paper-loop.md`. |
