...
| **3.4** Human | Compare `index.html` (2.4) with `index-polish.html` (QA, outlines off) + polish report. `?qa-outlines=tags` turns outlines on. Marking done promotes polish to `index.html` and archives the other homepage HTML. | Tidying before sign-off. Promoting before the compare. Writing `index.html` during 3.1–3.3. |
