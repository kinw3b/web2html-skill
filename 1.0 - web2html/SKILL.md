...
5. One `rebuild/`. 2.2 first pass is `index-semantic.html`. 2.4 lock is `index.html`; 3.x writes `index-polish.html` only (created when 3.1 starts, not at 2.4). Optional Phase 5 writes `astro/` and does not replace `rebuild/`. No local HTTP server. Preview is Chrome on `file://` — 5.3+ relativizes `astro/dist` so built pages open on `file://`. The agent verifies with `astro build`, never `astro dev`; the human may run `npm run preview`.
6. Human stops: **1.4**, **2.4**, **3.4**, optional **4.4**, optional **5.6**. Session 2 does not yield until **2.4**. **3.4** is finish vs Phase 4. **4.4** is finish vs Phase 5. **5.6** is finish (tidy). CTA only at those yields (Pitfall #190 #192).
...
| **2.3** | Pull numbered 1.2 `NN-slug.png` clips at 1600/768/390, side-by-side vs rebuild. Serial or **max 2**. **No Paper MCP.** Never skip. Then the **VALIDATE walk** top-to-bottom: `paper_23_validate.py . --next` → `--id <band> --shoot` → **Read** the three side PNGs → patch that band → `--record`; ≤3 rounds per band, then `--residual` (Pitfall #216). | same + `references/responsive-22d.md` + `references/section-23-paper-loop.md` |
...
| **5.2** | Serial `get_jsx` dumps + at most two author workers, each writing **only** the `<main>` of `astro/src/pages/{slug}.astro` on the 5.1 chrome. `record-phase-5-pages.py`. | same |
