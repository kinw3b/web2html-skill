...
## FIDELITY LOCK — when called from pipeline 3.1

If the parent pipeline is **`website-to-html`** (or the user says polish a Paper rebuild), this skill is **not** greenfield design:

1. **Do not redesign.** Layout, section order, spacing scale, type sizes, radii, Design Library class names, and brand colors are frozen at 2.4 (`qa/fidelity-freeze-24.json`).
2. **Dials:** force `redesign - preserve` / match existing. Do **not** run redesign-overhaul, invent a new aesthetic family, or swap fonts/palette/type size.
3. **In scope:** anti-AI-slop that does not move boxes (invented chrome, peeking skip-links, generic glow), focus rings on existing controls, contrast on existing tokens. **Out of scope:** Paper restore of list icons / checks / column `gap` / stack rhythm — that is 2.3 only (Pitfall #196). Load `website-to-html/references/polish-visual-restore.md`.
4. **Out of scope:** new hero composition, new card grids, new nav structure, GSAP pin/scrub, mandatory in-view reveal.
5. If the build does not match Paper / source screenshots, **stop and say so** — return to 2.3. Do not "taste" a signed page into looking different.
...
* **Animation:** **Motion**. Import from `motion/react` (`import { motion } from "motion/react"`). The `framer-motion` package still works as a legacy alias - prefer `motion/react` in new code.
...
For simple "items appear as they enter viewport" (no pinning) **in a React app**, prefer Motion's `whileInView` over GSAP - lighter, no ScrollTrigger needed. **On a website-to-html static `file://` rebuild, do not use Motion.** The canonical reveal is GSAP ScrollTrigger in-view stagger (`website-to-html/references/gsap-inview.md`), enforced on the Emil pass.
...
Use the Motion snippet above for React apps: feature lists, testimonial grids, logo walls, anything that just needs "enter on scroll." On website-to-html, Emil already ships that reveal via vendored GSAP. Save pin/scrub GSAP (5.A / 5.B) for motion the source actually recorded.
